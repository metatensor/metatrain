...
llpr_model.compute_covariance_as_pseudo_hessian(train_dataloader, target_info, loss_fn, parameters)
...
for batch in test_dataloader:
    ...
    predicted_forces = -torch.concatenate([system.positions.grad.flatten() for system in systems])
    ...
    for ll_feature in last_layer_features.reshape((-1,)):
        ...
        ll_feature_grad = torch.concatenate([ll_feature_g.flatten() for ll_feature_g in ll_feature_grad])
        ll_feature_grads.append(ll_feature_grad)
    ll_feature_grads = torch.stack(ll_feature_grads, dim=1)

    force_uncertainty = torch.einsum("if, fg, ig -> i", ll_feature_grads, exported_model._module.inv_covariance, ll_feature_grads)
