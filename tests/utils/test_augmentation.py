...
def test_replicate_and_augment_undo_round_trip():
    ...
    per-system reference vector, so ``undo_replicate_and_augment`` must map every
    one of the ``num_augmentations`` replicas of every system back to that exact
    reference vector."""
    target_name = "mtt::vectors"
    num_augmentations = 3
    systems = [
    ...
    back_transformed = rotational_augmenter.undo_replicate_and_augment(
