...
from metatensor.models.utils.device import string_to_devices
...
def test_string_to_device():
    ...
    # Test the CPU option.
    assert string_to_devices("cpu") == [torch.device("cpu")]

    # Test the CUDA option.
    if torch.cuda.is_available():
        assert string_to_devices("cuda") == [torch.device("cuda")]
    else:
        with pytest.raises(ValueError, match="CUDA is not available on this system"):
            string_to_devices("cuda")

    # Test the multi-GPU option.
    if torch.cuda.is_available():
        if torch.cuda.device_count() > 1:
            assert string_to_devices("multi-gpu") == [
    ...
    with pytest.raises(ValueError, match="Unrecognized device string `invalid-option`"):
        string_to_devices("invalid-option")
