...
def _make_carbon_only_batch():
    """
    A batch of carbon (type 6) only, no hydrogen. The global layout
    (``_make_layout_coupled_per_atom``) has both H and C, with H first, so for
    l=0 the global property union is n=[0,1,2]: H at positions [0,1], C at
    [1,2]. If C were treated as the only type present (i.e. using this
    batch's own target as the layout), its union would instead be n=[1,2] at
    positions [0,1], i.e. off by one from the global positions.
    """
    cell = torch.zeros((3, 3), dtype=torch.float64)
    pbc = torch.zeros(3, dtype=torch.bool)
    systems = [
    ...
    l1_comp = [
        Labels(
            names=["o3_mu"], values=torch.tensor([[-1], [0], [1]], dtype=torch.int32)
        )
    ]
    c_samples = Labels(
        names=["system", "atom"],
    ...
    c_l1 = torch.arange(12, dtype=torch.float64).reshape(2, 3, 2) + 400.0

    # Dense prediction (no atom_type key), padded over the global union: n=[0,1,2]
    # for l=0, n=[0,1] for l=1. l=0 column n=0 is H-only (999 here as a sentinel);
    # picking it up for carbon means the wrong (batch-local) union was used.
    prediction = TensorMap(
        ...
        blocks=[
            TensorBlock(
                values=torch.tensor(
                    [[[999.0, 21.0, 22.0]], [[999.0, 31.0, 32.0]]],
                    dtype=torch.float64,
                ),
                samples=c_samples,
                components=l0_comp,
                properties=Labels(
                    names=["n"], values=torch.tensor([[0], [1], [2]], dtype=torch.int32)
                ),
            ),
            TensorBlock(
                values=c_l1,
    ...
    # This batch's own target, with carbon only, no hydrogen key
    target = TensorMap(
        keys=Labels(
            names=["o3_lambda", "o3_sigma", "atom_type"],
            values=torch.tensor([[0, 1, 6], [1, 1, 6]], dtype=torch.int32),
        ),
        blocks=[
            TensorBlock(
                values=torch.tensor(
                    [[[21.0, 22.0]], [[31.0, 32.0]]], dtype=torch.float64
                ),
                samples=c_samples,
                components=l0_comp,
                properties=Labels(
                    names=["n"], values=torch.tensor([[1], [2]], dtype=torch.int32)
                ),
            ),
            TensorBlock(
                values=c_l1,
    ...
    return systems, prediction, target


def test_align_predictions_to_target_layout_uses_global_layout():
    """
    ``align_predictions_to_target_layout`` should sparsify using the target's
    global layout (``TargetInfo.layout``), not the batch's own target.
    """
    layout = _make_layout_coupled_per_atom()
    target_info = TargetInfo(layout=layout)
    systems, prediction, target = _make_carbon_only_batch()

    aligned = align_predictions_to_target_layout(
        systems,
        predictions={"target": prediction},
        targets={"target": target},
        target_info_dict={"target": target_info},
    )

    mts.allclose_raise(aligned["target"], target, atol=0.0, rtol=0.0)
