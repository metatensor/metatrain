...
def test_edges_single_bidirectional_roundtrip(batch_size):
    """
    Tests that the functions to get single direction edges and to recover the
    bidirectional edges work correctly by doing a round trip and checking that the data
    is the same as the original one.

    We load bidirectional data and we do: bidirectional -> single direction ->
    bidirectional.

    :param batch_size: The number of samples to load from the dataset for the test.
    """
