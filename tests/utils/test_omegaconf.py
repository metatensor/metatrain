...
def list_conf():
    ...
    conf = {
        "systems": system_section,
        "targets": {"energy": target_section, "my_target": target_section},
        "extra_data": {"ext::data": extra_data_section},
    }
...
def test_check_dataset_options_extra_data_unit(list_conf):
    """Test three datasets where the unit of the 2nd and the 3rd is inconsistent."""
    list_conf[1]["extra_data"]["ext::new_data"] = OmegaConf.create({"unit": "foo"})
    list_conf[2]["extra_data"]["ext::new_data"] = OmegaConf.create({"unit": "bar"})

    match = (
        "Units of extra_data section 'ext::new_data' are inconsistent. "
        "Found 'bar' and 'foo'!"
    )
