...
def systems_to_pyg_graphs(systems, options, all_species):
    ...
    for system in systems:
        ...
        S_list = [neighbors.samples['cell_shift_a'][None], neighbors.samples['cell_shift_b'][None], neighbors.samples['cell_shift_c'][None]]
        ...
        neighbor_index_constructor = NeighborIndexConstructor(i_list, j_list, S_list, species)
        neighbor_index_constructors.append(neighbor_index_constructor)
        
    max_nums = [neighbor_index_constructor.get_max_num() for neighbor_index_constructor in neighbor_index_constructors]
    max_num = max(max_nums)
    
    graphs = []
    for neighbor_index_constructor, system in zip(neighbor_index_constructors, systems):
        neighbors_pos, neighbors_index, nums, mask, neighbor_species, relative_positions = neighbor_index_constructor.get_neighbor_index(max_num, all_species)
        ...
        central_species = [np.where(all_species == specie)[0][0] for specie in np.array(system.species)]
