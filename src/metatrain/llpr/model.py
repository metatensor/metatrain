    ...
    def compute_covariance(
        ...
        with torch.no_grad():
            for batch in train_loader:
                ...
                outputs_for_targets = {
                    name: ModelOutput(sample_kind="atom" if "atom" in target.block(0).samples.names else "system")
    ...
    def calibrate(
        ...
        with torch.no_grad():
            for batch in valid_loader:
                ...
                requested_outputs = {}
                for name in targets:
                    sample_kind = "atom" if "atom" in targets[name].block(0).samples.names else "system"
                    requested_outputs[name] = ModelOutput(sample_kind=sample_kind)
                    uncertainty_name = _get_uncertainty_name(name)
                    requested_outputs[uncertainty_name] = ModelOutput(sample_kind=sample_kind)
