...
class FeedForward(nn.Module):
    def __init__(self, d_model: int, dim_feedforward: int, activation: str) -> None:
        ...
        if activation.lower() == "swiglu":
            ...
            self.w_in = nn.Linear(d_model, 2 * dim_feedforward, bias=False)
            self.w_out = nn.Linear(dim_feedforward, d_model, bias=False)
            self.activation = torch.nn.Identity()
            self.is_swiglu = True
        else:
            # Standard mode: regular activation function
            self.w_in = nn.Linear(d_model, dim_feedforward, bias=False)
            self.w_out = nn.Linear(dim_feedforward, d_model, bias=False)
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.input_linear = nn.Linear(total_dim, 3 * total_dim, bias=False)
        self.output_linear = nn.Linear(total_dim, total_dim, bias=False)
...
class TransformerLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        if norm == "LayerNorm":
            self.norm_attention = norm_class(d_model, bias=False)
            self.norm_mlp = norm_class(d_model, bias=False)
        else:
        ...
        if dim_node_features != d_model:
            ...
            self.center_contraction = nn.Linear(dim_node_features, d_model, bias=False)
            self.center_expansion = nn.Linear(d_model, dim_node_features, bias=False)
            if norm == "LayerNorm":
                self.norm_center_features = norm_class(dim_node_features, bias=False)
            else:
...
class CartesianTransformer(torch.nn.Module):
    ...
    def __init__(
        ...
        self.edge_embedder = nn.Linear(4, d_model, bias=False)
        ...
        self.compress = nn.Sequential(
            nn.Linear(n_merge * d_model, d_model, bias=False),
            torch.nn.SiLU(),
