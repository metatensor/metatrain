...
def test_lora_finetuning_device(device):
    ...
    model = FlashMD(MODEL_HYPERS, _get_dataset_info()).to(device)
...
def test_heads_finetuning_functionality():
    """Regression test: FlashMD's heads/last_layers live directly on the model
    (unlike PET, there is no ``backend`` submodule), so the ``head_modules`` and
    ``last_layer_modules`` prefixes must match FlashMD's own parameter names."""
    model = FlashMD(MODEL_HYPERS, _get_dataset_info())
