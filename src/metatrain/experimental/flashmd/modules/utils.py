    ...
    for system in systems:
        if "masses" not in system.known_data():
            ...
            masses_map = TensorMap(
                keys=Labels(names="_", values=torch.zeros((1, 1), dtype=torch.int32, device=values.device)),
                blocks=[
                    TensorBlock(
                        values=values,
                        samples=Labels(
                        ...
                        components=[],
                        properties=Labels(names="mass", values=torch.arange(1, device=values.device).unsqueeze(-1)),
                    )
                ],
            )
