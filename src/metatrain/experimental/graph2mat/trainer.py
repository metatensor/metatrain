...
from .utils.dataset import get_graph2mat_transform
from .utils.mtt import g2m_labels_to_tensormap


def filter_out_nans(
    targets: Dict[str, Any], predictions: Optional[Dict[str, Any]] = None
) -> None:
    """Filter out NaN values from the targets and predictions.
    ...
    clean_targets = {}
    clean_predictions = {} if predictions is not None else None
    for key in targets.keys():
        target_tensormap = targets[key]
        filtered_targets = []
        filtered_predictions = []
        for i, block in enumerate(target_tensormap.blocks()):
            mask = torch.isnan(block.values)
            filtered_targets.append(block.values[~mask])
            if predictions is not None:
                filtered_predictions.append(predictions[key].block(i).values[~mask])

        clean_targets[key] = g2m_labels_to_tensormap(
            node_labels=filtered_targets[0],
            edge_labels=filtered_targets[1],
        )

        if predictions is not None:
            clean_predictions[key] = g2m_labels_to_tensormap(
                node_labels=filtered_predictions[0],
                edge_labels=filtered_predictions[1],
            )

    return clean_targets, clean_predictions
...
class Trainer(TrainerInterface[TrainerHypers]):
    ...
    def train(
        ...
        assert dtype in MetaGraph2Mat.__supported_dtypes__
        ...
        # The additive models are always in float64 (to avoid numerical
        ...
        collate_fn = CollateFn(
            ...
            callables=[
                ...
                get_graph2mat_transform(
                    model.graph2mat_processors, model.graph2mat_nls
                ),
            ],
        )
        ...
        optimizer = getattr(torch.optim, self.hypers["optimizer"])(
            model.parameters(), **self.hypers["optimizer_kwargs"]
        )

        lr_scheduler = None
        if self.hypers["lr_scheduler"] is not None:
            lr_scheduler = getattr(
                torch.optim.lr_scheduler, self.hypers["lr_scheduler"]
            )(
                optimizer,
                **self.hypers["lr_scheduler_kwargs"],
            )
        ...
        logging.info(f"Base learning rate: {optimizer.param_groups[0]['lr']}")
