...
def find_common_neighbors(data):
    ...
    for i_edge, (center, neigh) in enumerate(edges):
        ...
        # same third atom
        for i, center_edge, center_shift in zip(
            center_iedges, center_edges, center_shifts
        ):
            for j, neigh_edge, neigh_shift in zip(
                neighbor_iedges, neigh_edges, neigh_shifts
            ):
                if neigh_edge[1] == center_edge[1] and torch.all(
                    center_shift == neigh_shift
                ):
                    common_neighs.append([i_edge, i, j])
...
class E3nnTwoCenterMessageBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        # Convolution weights
        assert edge_feats_irreps.lmax == 0, (
    ...
    def forward(
        ...
        if self.common_neighs is None or not torch.all(
            data["positions"] == self.common_neighs_data
        ):
        ...
        two_center_edge_feats = (
            edge_feats[first_center_edge] * edge_feats[second_center_edge]
        )
        ...
        i_edge = torch.concat(
            [i_edge, torch.arange(data["edge_index"].shape[1], device=i_edge.device)],
            dim=0,
        )
        ...
        two_center_msgs = self.conv_tp(neigh_feats, edge_attrs, tp_weights)
        ...
        edge_values = torch.zeros(
            (data["edge_index"].shape[1], two_center_msgs.shape[-1]),
            device=two_center_msgs.device,
        )
...
class E3nnLinearNodeBlock(torch.nn.Module):
    ...
    def __init__(self, irreps_in: o3.Irreps, irreps_out: o3.Irreps):
        ...
        if isinstance(irreps_in, (list, tuple)) and not isinstance(
            ...
            assert all(irreps == irreps_in[0] for irreps in irreps_in), (
