...
def g2m_labels_to_tensormap(
    ...
    node_tmap = TensorMap(
        keys=Labels(
            ["_"],
            torch.tensor([[0]], device=node_labels.device),
        ),
        blocks=[
            _wrap_in_tensorblock(node_labels, i, dtype=dtype),
        ],
    )
    edge_tmap = TensorMap(
        keys=Labels(
            ["_"],
            torch.tensor([[0]], device=edge_labels.device),
        ),
        blocks=[
            _wrap_in_tensorblock(edge_labels, i, dtype=dtype),
        ],
    )
    return node_tmap, edge_tmap
...
def split_dataset_info(
    ...
    for matrix_name, matrix_spec in matrices.items():
        ...
        graph2mat_targets[edge_target] = dataset_info.targets[edge_target]

    for target_name, target_info in dataset_info.targets.items():
        if target_name not in graph2mat_targets:
