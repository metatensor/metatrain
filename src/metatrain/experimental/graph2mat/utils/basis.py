...
def get_basis_from_layout(layout: TensorMap, R: float) -> AtomicTableWithEdges:
    ...
    for key, block in layout.items():
        ...
        if is_coupled:
            ...
            for l in unique_ls:
                max_n = block.properties["n_1"][block.properties["l_1"] == l].max().item()
                num_per_l[atom_type][l.item()] = max(num_per_l[atom_type].get(l.item(), 0), max_n + 1)
        else:
            l = key["o3_lambda_1"]
            max_n =  block.properties["n_1"].max().item() # All samples in the block have the same l
