...
def system_to_config(
    ...
    for i in range(3):
        if not ase_atoms.pbc[i]:
            ase_atoms.cell[i, i] = ase_atoms.positions[:, i].max() - ase_atoms.positions[:, i].min() + 20.0
...
def get_graph2mat_transform(
        ...
        for key, block in tmap.items():
            ...
            blocks.append(TensorBlock(
                ...
                samples=Labels(
                    ...
                    values=torch.tensor([[0], [1]], dtype=torch.int32, device=values.device),
                ),
    ...
    for matrix_name, matrix_spec in matrices.items():
        ...
        layouts[matrix_name] = {
            "nodes": get_tmap_prop_to_matrix(orb_pointers[matrix_name], node_target_info.layout),
            "edges": get_tmap_prop_to_matrix(orb_pointers[matrix_name], edge_target_info.layout),
        }
    ...
    def transform(
        ...
        system_indices = (
            extra["mtt::aux::system_index"].block(0).values.ravel().to(torch.int64).tolist()
        )

        def convert_tmaps(matrix_name, nodes_tmap, edges_tmap, systems, add_neighbor_list=True):
            ...
            for key, block in nodes_tmap.items():
                ...
                mat_block_shape = tuple(basis_table.point_block_shape[:, basis_table.type_to_index(atom_type)])
                ...
                for i, sample in enumerate(block.samples.values):
                    ...
                    if entry_key not in system_matrix:
                        system_matrix[entry_key] = torch.full(mat_block_shape, torch.nan, dtype=block.values.dtype, device=block.values.device)

                    system_matrix[entry_key][property_to_matrix[0], property_to_matrix[1]] = block.values[i]
            ...
            for key, block in edges_tmap.items():
                ...
                edge_type = basis_table.point_type_to_edge_type([[basis_table.type_to_index(at_type_1)], [basis_table.type_to_index(at_type_2)]])[0]
                ...
                for i_pair, (
                    ...
                    *cell_shifts,
                    ...
                    if entry_key not in block_dict_matrices[int(i_system)]:
                        block_dict_matrices[int(i_system)][entry_key] = torch.full(mat_block_shape, torch.nan, dtype=block.values.dtype, device=block.values.device)

                    block_dict_matrices[int(i_system)][entry_key][property_to_matrix[0], property_to_matrix[1]] = block.values[i_pair]

            configs = [
                system_to_config(
                    system, graph2mat_processors[matrix_name], block_dict_matrices[i], nsc=lattices[i].nsc
                )
        ...
        for matrix_name, matrix_spec in matrices.items():
            ...
            extra[node_scales], extra[edge_scales] = convert_tmaps(matrix_name, extra[node_scales], extra[edge_scales], systems, add_neighbor_list=False)
            ...
            extra[node_per_property_scales], extra[edge_per_property_scales] = convert_tmaps(matrix_name, extra[node_per_property_scales], extra[edge_per_property_scales], systems, add_neighbor_list=False)
...
def graph2mat_to_tensormap(
    ...
    for i_system in range(len(batch["ptr"]) - 1):
        ...
        atom_ptrs = processor.basis_table.atom_block_pointer(atom_types)

        atom_Zs = torch.tensor(processor.basis_table.types)[atom_types].tolist()

        for i_atom, atom_Z in enumerate(atom_Zs):
            start, end = atom_start + atom_ptrs[i_atom: i_atom + 2]
    ...
    for k, blocks in node_blocks.items():
        shape = processor.basis_table.point_block_shape[:, processor.basis_table.type_to_index(k)]
    ...
    for k, blocks in edge_blocks.items():
        ...
        new_edge_blocks[(at_Z_1, at_Z_2)] = torch.stack(blocks, dim=0).reshape(-1, *shape)
    ...
    n_ls = {
        point_basis.type: [mul for mul, l, sigma in point_basis.basis] for point_basis in
        processor.basis_table.basis
    }
    # Cumsums
    ...
    def get_tmap(input_blocks: dict, input_samples: dict):
        ...
        if is_atom:
            keys_names = ["o3_lambda_1", "o3_lambda_2", "o3_sigma_1", "o3_sigma_2", "atom_type"]
            samples_names = ["system", "atom"]
        else:
            keys_names = ["o3_lambda_1", "o3_lambda_2", "o3_sigma_1", "o3_sigma_2", "first_atom_type", "second_atom_type"]
            samples_names = ["system", "first_atom", "second_atom", "cell_shift_a", "cell_shift_b", "cell_shift_c"]
