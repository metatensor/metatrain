...
class Trainer:
    ...
    def train(
        ...
        assert dtype in MLElec.__supported_dtypes__
        ...
        if is_distributed:
            if len(devices) > 1:
                raise ValueError(
                    ...
                    " If you want to run distributed training with MLElec, please "
                    "set `device` to cuda."
                )
        ...
        # Pretrain the model:
        logging.info("Pre-training the model with linear regression")
        model.pretrain(train_dataloader)
    ...
    def save_checkpoint(self, model, path: Union[str, Path]):
        checkpoint = {
            "architecture_name": "experimental.mlelec",
            "model_data": {
