...
from metatrain.experimental.flashmd_symplectic.checkpoints import model_update_v2_v3
...
def _make_checkpoint() -> dict:
    """
    Build a minimal v2 checkpoint with deprecated plural names in every
    location the migration must rewrite
    """
...
def test_model_update_v2_v3_renames_targets_loss_and_layout():
    checkpoint = _make_checkpoint()
    model_update_v2_v3(checkpoint)
...
def test_model_update_v2_v3_renames_state_dict_keys_and_scaler_buffers():
    checkpoint = _make_checkpoint()
    model_update_v2_v3(checkpoint)
