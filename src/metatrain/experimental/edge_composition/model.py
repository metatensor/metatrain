...
from .radials import Exponential
...
SPHERICART_NORMS = torch.tensor([0.2821, 0.4886, 0.6308, 0.7464, 0.8463, 0.9356, 1.0171, 1.0925, 1.1631, 1.2296, 1.2927])
...
class EdgeCompositionModel(ModelInterface[ModelHypers]):
    ...
    def _add_output(self, target_name: str, target_info: TargetInfo) -> None:
        ...
        for (type1, type2), n_props in n_props_per_type_pair.items():
            self.radials[target_name][str((type1, type2))] = Exponential(n_props)
    ...
    def load_checkpoint(
        ...
        if not "spherical_harmonics._factors" in checkpoint["model_state_dict"]:
