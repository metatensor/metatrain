...
class Encoder(torch.nn.Module):
    ...
    def forward(
        ...
        # Encode angles
        spherical_harmonics = self.spherical_harmonics(features["cartesian"])  # [n_edges, (max_angular + 1) ** 2]

        # Encode distances (Spherical Bessel with l = 0 for simplicity)
        distances = torch.sqrt(torch.sum(features["cartesian"] ** 2, dim=-1))  # [n_edges]
        scaled_distances = torch.stack([
            distances * n * torch.pi / self.cutoff_radius
            for n in range(1, self.max_radial + 1)
        ], dim=-1)  # [n_edges, max_radial]    
