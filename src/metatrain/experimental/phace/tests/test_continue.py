...
def test_continue(monkeypatch, tmp_path):
    ...
    output_before = scripted_model(systems[:5], {"mtt::U0": model.outputs["mtt::U0"]})
    ...
    scripted_model_before = torch.jit.script(model_before)
    scripted_model_after = torch.jit.script(model_after)
    ...
    output_after = scripted_model_after(
