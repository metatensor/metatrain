...
class TensorProduct(torch.nn.Module):
    def __init__(self, k_max_l: List[int]):
        ...
        for padded_l in list(U_dict.keys()):
            ...
            U_dict_parity[f"{padded_l}_{1}"] = U_dict_parity[f"{padded_l}_{1}"].to_sparse()
            print("Sparsity: ", U_dict_parity[f"{padded_l}_{1}"]._nnz() / U_dict_parity[f"{padded_l}_{1}"].numel())
            U_dict_parity[f"{padded_l}_{-1}"] = U_dict[padded_l].clone()
