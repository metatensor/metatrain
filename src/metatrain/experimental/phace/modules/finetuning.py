...
class FullFinetuneHypers(TypedDict):
    """Hyperparameters to use full finetuning of PhACE models.
...
class LoRaFinetuneHypers(TypedDict):
    """Hyperparameters for LoRA finetuning of PhACE models.
...
class HeadsFinetuneHypers(TypedDict):
    """Hyperparameters for heads finetuning of PhACE models.
        ...
        if not lora_already_applied:
            ...
            model = inject_lora_layers(
                ...
                target_modules=tuple(
                    lora_config.get("target_modules", ["linear_layer"])
                ),
...
def inject_lora_layers(
    model: nn.Module,
    target_modules: Tuple[str, ...] = ("linear_layer",),
    rank: int = 4,
    alpha: float = 1.0,
    ...
    :param target_modules: A tuple of strings specifying the names of the attributes of
        the modules to be replaced with LoRA layers. For PhACE, the default is
        ``("linear_layer",)`` which targets the inner ``nn.Linear`` inside each PhACE
        ``Linear`` module.
