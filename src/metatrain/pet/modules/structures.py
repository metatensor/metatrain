...
def systems_to_batch(
    ...
    :return: A tuple containing the batch tensors.
        ...
        - `atomic_cutoffs_stats`: Diagnostic per-atom adaptive cutoffs (detached
            from the autograd graph). Empty tensor if `num_neighbors_adaptive` is None.
        - `num_neighbors_stats`: Diagnostic per-atom neighbor count after pruning.
