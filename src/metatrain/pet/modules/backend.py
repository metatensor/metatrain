...
# The only keys a ``readout_type`` spec dict (bare, or as one of a per-target
# mapping's values) may carry -- see ``_check_readout_type`` and
# ``_make_readout``.
_READOUT_SPEC_KEYS = ("atom_type_gating", "hypers")


def _check_readout_type(readout_type: Any) -> None:
    """Raise if ``readout_type`` (or, when it is a per-target mapping, any of its
    values) is a spec dict carrying a key outside :data:`_READOUT_SPEC_KEYS`.

    A misspelled key (``atomic_type_gating`` for ``atom_type_gating`` is the one
    that has actually bitten us) is not caught anywhere else: it never satisfies
    :func:`resolve_per_target`'s ``spec_keys`` disambiguation, so the dict is
    silently read as an (empty, for any real target name) per-target mapping
    instead of the intended bare spec, ``atom_type_gating`` quietly falls back to
    its default, and nothing raises. This is called both where a target's readout
    is actually built (:meth:`PETBackend.add_output`, covering fresh training and
    any newly-added target) and on the raw hyper of a restart/finetune run before
    it can be silently ignored for a target that already existed
    (:meth:`metatrain.pet.model.PET.restart`).
    """
    if not isinstance(readout_type, dict):
        return
    is_bare_spec = any(key in readout_type for key in _READOUT_SPEC_KEYS)
    if is_bare_spec:
        candidates = [readout_type]
    else:
        # A genuine per-target mapping maps target names to spec dicts, so
        # its values should themselves be dicts. If none of them are, this
        # cannot actually be a per-target mapping either -- it is a
        # malformed (most likely misspelled) bare spec instead, and
        # ``readout_type`` itself is what to check.
        dict_values = [v for v in readout_type.values() if isinstance(v, dict)]
        candidates = dict_values if dict_values else [readout_type]
    for spec in candidates:
        if not isinstance(spec, dict):
            continue
        unknown = sorted(set(spec) - set(_READOUT_SPEC_KEYS))
        if unknown:
            raise ValueError(
                f"Unknown key(s) in 'readout_type': {unknown}. Allowed keys "
                f"are {_READOUT_SPEC_KEYS}. (if you meant to set the gating, "
                "the key is 'atom_type_gating', not 'atomic_type_gating')"
            )
...
class PETBackend(torch.nn.Module):
    ...
    def __init__(self, hypers: ModelHypers, atomic_types: List[int]) -> None:
        ...
        _check_readout_type(self.readout_type)
