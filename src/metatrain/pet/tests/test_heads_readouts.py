...
def test_unknown_readout_type_key_raises():
    """A misspelled key (the real one that has bitten us: 'atomic_type_gating' for
    'atom_type_gating') must be rejected outright, not silently misread as a
    per-target mapping with no entry for any real target -- which is what
    ``resolve_per_target`` would otherwise do, quietly falling back to
    ``atom_type_gating: False`` with no error at all.
    """
    with pytest.raises(ValueError, match="Unknown key.*readout_type"):
        PET(_hypers(readout_type={"atomic_type_gating": "one-hot"}), _dataset_info())


def test_unknown_readout_type_key_raises_per_target():
    """Same typo, one level down: readout_type keyed by target name."""
    with pytest.raises(ValueError, match="Unknown key.*readout_type"):
        PET(
            _hypers(readout_type={TARGET: {"atomic_type_gating": "one-hot"}}),
            _dataset_info(),
        )


def test_restart_catches_unknown_readout_type_key_for_an_existing_target():
    """The scenario that actually happened: a misspelled readout_type key in a
    finetuning run's own hypers, for a target that already existed in the
    checkpoint being finetuned from. ``add_output`` (where the same check also
    runs) is never called again for an existing target, so without an explicit
    check in ``restart`` itself, this typo would be silently ignored -- the
    checkpoint's own (correctly-spelled, already-built) readout is kept exactly
    as it was, and training proceeds without any error.
    """
    model = PET(_hypers(readout_type={"atom_type_gating": "one-hot"}), _dataset_info())
    with pytest.raises(ValueError, match="Unknown key.*readout_type"):
        model.restart(
            _dataset_info(),
            model_hypers=_hypers(readout_type={"atomic_type_gating": "one-hot"}),
        )
