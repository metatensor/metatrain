...
def model_update_v11_v12(checkpoint: dict) -> None:
    ...
    Also injects defaults for ``cutoff_max_adaptive`` (None → falls back to
    ``cutoff`` at __init__) and ``cutoff_width_adaptive`` (set to the
    ...
    if "cutoff_max_adaptive" not in hypers:
        hypers["cutoff_max_adaptive"] = None
