...
from metatrain.utils.matrix import Blocks2Matrix, read_basis_set
...
class LossInterface(ABC):
    ...
    def __call__(
        ...
        :return: Value of the loss.
        """
        return self.compute(predictions, targets, systems, extra_data)
...
class LossAggregator(LossInterface):
    ...
    def set_epoch(self, epoch: int):
        for term in self.losses.values():
            if hasattr(term, "set_epoch"):
                term.set_epoch(epoch)
    def compute(
        self,
        predictions: Dict[str, TensorMap],
        targets: Dict[str, TensorMap],
        ...
        for term in self.losses.values():
            ...
            total_loss = total_loss + term.weight * term.compute(
                predictions, targets, systems, extra_data
            )
...
class EigenvalueLoss(LossInterface):
    """
    Loss function that combines Hamiltonian MSE and eigenvalue MSE.
    For the first `mse_warmup_epochs` epochs, uses a plain MSE loss on the
    Hamiltonian blocks. Every `mse_warmup_epochs + 1` epochs, switches to
    an eigenvalue loss that computes the generalized eigenvalues of the
    predicted and reference Hamiltonians.

    :param name: key in the predictions/targets dict.
    :param gradient: optional gradient field name.
    :param weight: weight of the loss contribution in the final aggregation.
    :param reduction: reduction mode for torch loss.
    :param basis: path to the basis set file.
    :param device: device to use for computation.
    :param target: key in the predictions/targets dict for the Hamiltonian.
    :param xyz_path: path to the xyz file containing the dataset.
    :param mse_warmup_epochs: number of MSE epochs before switching to eigenvalue loss.
    """
    ...
    def __init__(
        ...
        xyz_path: str = "/scratch/saade/qm7_diverse.xyz",
        mse_warmup_epochs: int = 50,
    ):
        super().__init__(
            name=name,
            gradient=gradient,
        ...
        self.mse_loss = TensorMapMaskedMSELoss(
            name=self.target,
            gradient=None,
            weight=self.weight,
        ...
        self._parse_xyz_fingerprints(xyz_path)
        ...
        self.precomputed_dir = "/scratch/saade/qm7_diverse_exps/matrices"
        self.ref_cache = {}
        for i in range(1000):
            folder = os.path.join(self.precomputed_dir, str(i))
            H = torch.load(os.path.join(folder, f"H{i}.pt"))
            S = torch.load(os.path.join(folder, f"S{i}.pt"))
            self.ref_cache[i] = (H, S)

    def set_epoch(self, epoch: int):
        self.current_epoch = epoch

    def _parse_xyz_fingerprints(self, xyz_path: str):
        ...
        while i < len(lines):
            ...
            for _ in range(n_atoms):
                ...
                i += 1
            fp = self._make_fingerprint(species, positions)
            self.fingerprint_to_id[fp] = frame_idx
            frame_idx += 1

    def _make_fingerprint(self, species, positions):
        if isinstance(species, torch.Tensor):
            z_to_element = {1: "H", 6: "C", 7: "N", 8: "O"}
            species = [z_to_element[z.item()] for z in species]
        if isinstance(positions, torch.Tensor):
            positions = positions.cpu().tolist()
        pairs = sorted(zip(species, positions))
        sp = tuple(p[0] for p in pairs)
        pos = tuple(round(x, 3) for p in pairs for x in p[1])
        return (sp, pos)

    def generalized_eigvals(self, H, S):
        H_op = xitorch.LinearOperator.m(H, is_hermitian=True)
        S_op = xitorch.LinearOperator.m(S, is_hermitian=True)
        evals, _ = symeig(H_op, M=S_op)
        return evals
    ...
    def compute(
        ...
        use_eig = (
            (self.current_epoch % (self.mse_warmup_epochs + 1)) == self.mse_warmup_epochs
        )

        if use_eig:
            H_pred_list = self.b2m(systems, predictions[self.target])
            losses = []

            for H_pred_dict, system in zip(H_pred_list, systems):
                H_pred = H_pred_dict["0_0_0"]
                device, dtype = H_pred.device, H_pred.dtype

                fp = self._make_fingerprint(system.types, system.positions)
                sys_id = self.fingerprint_to_id[fp]

                H_ref_cpu, S_ref_cpu = self.ref_cache[sys_id]
                H_ref = H_ref_cpu.to(device=device, dtype=dtype)
                S_ref = S_ref_cpu.to(device=device, dtype=dtype)

                eig_pred = self.generalized_eigvals(H_pred, S_ref)
                eig_ref  = self.generalized_eigvals(H_ref, S_ref)

                ham_loss = (H_pred - H_ref).pow(2).mean()
                eig_loss = (eig_pred - eig_ref).pow(2).mean()
                losses.append(ham_loss + eig_loss)

            if len(losses) == 0:
                return torch.zeros((), device=systems[0].positions.device)

            loss = torch.stack(losses)
            return loss.mean() if self.reduction == "mean" else loss.sum()
        else:
            return self.mse_loss.compute(predictions, targets, extra_data)
