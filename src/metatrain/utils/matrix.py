...
class Blocks2MatrixCompiled(torch.nn.Module):
    """
    Non-differentiable compiled version of Blocks2Matrix for transforming 
    Hamiltonian coupled blocks into dense matrices without gradient computation.
    
    This version is optimized for targets and overlaps where gradients are not 
    needed, avoiding unnecessary gradient tracking.
    ...
    def forward(
        ...
        """
        Apply the block-to-matrix transformation without gradients.
        ...
        :param mask: Optional TensorMap mask indicating valid entries.
        :return: A list (over systems) of dictionaries mapping cell shifts to dense
            matrix tensors (detached from gradient graph).
        """
        with torch.no_grad():
            device = tensor[0].values.device
            dtype = tensor[0].values.dtype
