            ...
            for key, block in target.items():
                if self.sample_kinds[target_name] == "per_structure":
                    ...
                    if "non_conservative_stress" in target_name:
                        ...
                        valid_mask = (~torch.isnan(Y)).to(Y.dtype)
                        Y = torch.nan_to_num(Y, nan=0.0)
                        dim = list(range(0, Y.dim() - 1))
                        N = torch.sum(valid_mask, dim=dim)
                        Y2_values = torch.sum((Y * valid_mask) ** 2, dim=dim)
                    else:
                    ...
                    if "atom_type" in key.names:
                        ...
                        # Here it is assumed that the samples of the block correspond to
                        # the full ordered list of atoms in the batch of systems
