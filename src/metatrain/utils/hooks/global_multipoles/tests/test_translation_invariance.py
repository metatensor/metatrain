...
def _random_systems(generator: torch.Generator) -> list[System]:
    ...
    for size in SYSTEM_SIZES:
        ...
        positions = torch.rand(
            (size, 3), generator=generator, dtype=torch.float64
        ) * 4.0 + torch.tensor([7.0, -3.0, 11.0], dtype=torch.float64)
        systems.append(
            System(
                types=types,
                positions=positions,
...
def _random_local_multipoles(
    ...
    blocks = [
        ...
        TensorBlock(
            ...
            samples=samples,
            components=[Labels(names=["o3_mu"], values=torch.tensor([[-1], [0], [1]]))],
            properties=Labels(names=["properties"], values=torch.tensor([[0]])),
        ),
    ]
...
def test_matches_centre_of_nuclear_charge(hook, outputs, generator):
    ...
    for system in systems:
        ...
        origin = (nuclear_charges.unsqueeze(1) * positions).sum(
            0
        ) / nuclear_charges.sum()
