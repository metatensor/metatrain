...
class O3Augmenter:
    ...
    def replicate_and_augment(
        ...
        device = replicated[0].positions.device
        if self._group == "inversions":
            signs = torch.randint(0, 2, (len(replicated),), device=device) * 2 - 1
            matrices = torch.stack(
                [sign * torch.eye(3, dtype=dtype, device=device) for sign in signs]
            )
            transformations = O3Transformations(matrices, self._max_angular_momentum)
        else:
