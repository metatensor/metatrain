...
class CartesianTransformer(torch.nn.Module):
    ...
    def __init__(
        ...
        self.edge_embedder = nn.Linear((self.l_max + 1) ** 2 + self.n_rad, d_model)
    ...
    def forward(
        ...
        sph = self.spherical_harmonics(edge_vectors.reshape(-1, 3)).reshape(
            edge_vectors.shape[0], edge_vectors.shape[1], -1
        )

        # Compute Bessel RBFs
        eps = 1e-8
        r = (edge_distances / self.cutoff).clamp(0.0, 1.0)
        x = (
            torch.pi
            * r[:, :, None]
        ...
        edge_embeddings = torch.concat([sph, rbf], dim=-1)
