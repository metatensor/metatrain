...
class ZConditionedMLP(torch.nn.Module):
    """
    Two-layer species-conditioned readout MLP.
    """
    ...
    def __init__(
        ...
        # Layer 1: per-species linear (batched bmm), followed by nonlinearity.
        self.z_linear_1 = ZConditionedLinear(
            n_species=n_species,
            in_features=in_features,
            out_features=hidden_features,
        )
        self.activation = torch.nn.SiLU()
        # Layer 2: per-species linear (batched bmm) directly to output.
        self.z_linear_2 = ZConditionedLinear(
            n_species=n_species,
    ...
    def forward(
        ...
        """
        Node path  : (n_atoms, in_features)           → (n_atoms, out_features)
        Edge path  : (n_atoms, n_nbrs, in_features)   → (n_atoms, n_nbrs, out_features)

        The dim==2 vs dim==3 branch is handled inside ZConditionedLinear.forward.
        """
        h = self.activation(self.z_linear_1(features, species_idx))
        return self.z_linear_2(h, species_idx)
                ...
                torch.nn.ModuleDict(
                    {
                        key: ZConditionedMLP(
                            ...
                            # hidden_features defaults to d_head (no bottleneck).
                        )
        ...
        for layer_index in range(self.num_readout_layers):
            for key in self.output_shapes[target_name].keys():
                self.last_layer_parameter_names[target_name].append(
                    f"node_last_layers.{target_name}.{layer_index}.{key}.z_linear_2.weight"
                )
                self.last_layer_parameter_names[target_name].append(
                    f"edge_last_layers.{target_name}.{layer_index}.{key}.z_linear_2.weight"
                )
