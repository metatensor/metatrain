...
data is available. You find the dataset and files for this tutorial in the 
`github repository <examples/fine-tuning>`.
...
Let's assume you are in an environent where you installed ``metatrain``.
...
#   training_set:
#     systems:
#         read_from: metatrain/examples/ase/ethanol_reduced_100.xyz
#         reader: ase
...
#
#
# After setting up your ``options.yaml`` file, we can run our fine-tuning:
# .. code-block:: bash
#   mtt train options.yaml
#
# The training metrics for your run will be saved in ``outputs/YYYY-MM-DD/HH-MM-SS/train.csv``. You can run a short 
# python script to check your fine-tuning performance. Here we ran only for 20 epochs.
# 
