...
.. warning::
  ...
  only available for the PET, FlashMD and FlashMDSymplectic models.
...
**after** training for the current run has finished (right before the final
checkpoint is saved), so ``energy/variant`` must already be fully trained -- either
