...
For heterogeneous datasets, missing supervision can be represented by ``NaN`` values in the targets. Pointwise losses ignore those unsupported entries. Invariant losses apply the same policy at the entity level: if a supervised vector/tensor entity has missing target components, that entity is excluded from the invariant reduction. When a metric has zero valid support over an epoch, it is omitted from the logged metrics rather than reported as ``NaN``.
