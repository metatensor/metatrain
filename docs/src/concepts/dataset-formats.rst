...
- ``pbc.npy`` (optional): boolean periodic boundary flags, shape ``(ns, 3)``, with one
  flag per cell vector. Cell vectors corresponding to ``False`` are set to zero when
  loading. If this file is missing, periodicity is inferred from the cell vectors (each
  non-zero vector defines a periodic direction).
